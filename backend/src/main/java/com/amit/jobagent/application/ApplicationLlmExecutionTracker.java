...
class ApplicationLlmExecutionTracker {
    ...
    UUID started(String operation, String input, ApplicationContentGenerationRequest source) {
        ...
        jdbc.update("""
                ...
                """, id, operation, "OPENAI", config.model(), operation.toLowerCase(Locale.ROOT),
    ...
    void succeeded(
        ...
        jdbc.update("""
                ...
                """, providerResponseId, Instant.now(), latencyMilliseconds, inputTokens, outputTokens,
                totalTokens, checksum(output), id);
    }

    void failed(UUID id, Throwable failure, long latencyMilliseconds, boolean refusal) {
        String code = failure.getClass().getSimpleName().replaceAll("[^A-Za-z0-9_]", "_").toUpperCase(Locale.ROOT);
        if (code.isBlank()) code = "PROVIDER_FAILURE";
        jdbc.update("""
                ...
                """, Instant.now(), latencyMilliseconds, limit(code, 80),
                "Application content provider request failed safely", refusal,
