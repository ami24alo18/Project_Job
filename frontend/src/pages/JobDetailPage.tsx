...
export function JobDetailPage() {
  ...
  const generateApplicationDraft = async () => {
    if (!job?.descriptionPlainText?.trim()) {
      setMessage('')
      setError('Add the complete job description with Edit job, save it, and evaluate the updated job before generating a draft. No model request was sent.')
      return
    }
    ...
    try {
      ...
      setError(apiErrorMessage(problem, 'An application draft could not be generated. Activate a text-based resume and evaluate this job before generating a draft.'))
  ...
  return <Stack spacing={3}>
    ...
    {!job.descriptionPlainText?.trim() && !form && <Alert severity="warning" action={<Button color="inherit" onClick={() => setForm(jobToForm(job))}>Add job description</Button>}>This job has no description. Evaluation and résumé tailoring need the complete JD; draft generation is blocked to avoid wasting model tokens.</Alert>}
